<div class="wrap">
	...
	<div id="poststuff">
		<form id="em-types-form" method="post">
			...
			<input type="hidden" id="em-types-ajax-nonce" name="em_types_ajax_nonce" value="<?= wp_create_nonce('em-types-check-slug'); ?>" />
			<input type="hidden" id="em-types-type" name="em_types[type]" value="<?= self::get_value('type'); ?>" />
			<input type="hidden" id="em-types-slug" name="em_types[slug]" value="<?= self::get_value('slug'); ?>" />
			<div id="titlediv">
				<div id="titlewrap">
					<label for="em-types-title" id="title-prompt-text"<?= self::get_value('name') != '' ? ' class="screen-reader-text"' : ''; ?>>Enter name (plural version) here</label>
			...
			<table class="form-table">
				<tbody>
					...
					<tr valign="top" data-for="post-type"<?= ( self::get_value('type') == 'post-type' ) ? '' : 'class="hidden"'; ?>>
						<th scope="row"><label for="em-types-menu-position">Menu Position</label></th>
						<td>
					...
					<tr valign="top" data-for="post-type"<?= ( self::get_value('type') == 'post-type' ) ? '' : 'class="hidden"'; ?>>
						<th scope="row"><label for="em-types-supports">Supports</label></th>
						<td>
							<?php foreach ( self::$supports as $item ) : ?>
							<p><label for="em-types-supports-<?= $item ?>">
					...
					<tr valign="top" data-for="all">
						<th scope="row"><label>Public</label></th>
						<td>
							<label for="em-types-public">
					...
					<tr valign="top" data-for="all">
						...
						<td>
							<label for="em-types-exclude-from-search">
								<input type="checkbox" id="em-types-exclude-from-search" name="em_types[exclude_from_search]" value="1"<?= self::get_value('exclude_from_search') == 1 ? ' checked' : ''; ?> />
								Yes</label>
						</td>
					</tr>
					<tr valign="top" data-for="all">
					...
					<tr valign="top" data-for="taxonomy"<?= ( self::get_value('type') == 'taxonomy' ) ? '' : 'class="hidden"'; ?>>
					...
					<tr valign="top" data-for="post-type"<?= ( self::get_value('type') == 'post-type' ) ? '' : 'class="hidden"'; ?>>
						<th scope="row"><label>Menu Icons</label> From <a href="https://developer.wordpress.org/resource/dashicons" target="_blank">Wordpress Dashicons</a></th>
						<td>
							<input type="text" id="em-types-dashicon" class="regular-text" name="em_types[menu_icon]" value="<?= self::get_value('menu_icon'); ?>" />
						</td>
					</tr>
					<tr valign="top" data-for="taxonomy"<?= ( self::get_value('type') == 'taxonomy' ) ? '' : 'class="hidden"'; ?>>
						...
						<td>
							...
							<p>
								<label for="em-types-pt-<?= $pt->name; ?>">
									<input type="checkbox" name="em_types[post_type][]" id="em-types-pt-<?= $pt->name; ?>" value="<?= $pt->name; ?>"<?= in_array($pt->name, self::get_value('post_type', array())) ? ' checked' : ''; ?> />
									<?= $pt->label; ?>
								</label>
							</p>
							<?php endforeach; ?>
			...
			<p class="submit"><input type="submit" value="Save Changes" class="button button-primary" id="em-types-submit" name="submit" /></p>
		</form>
	</div>
</div>